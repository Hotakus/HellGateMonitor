...
void HgmGUI::HgmSetupUI::Begin()
{
    ...
    lv_bar_set_range(pb, 0, (HGM_COMPONENT_NULL * 1000));
    ...
    for (size_t i = 0; i < HGM_COMPONENT_NULL; i++) {
        ...
        vTaskDelay(600);
        this->ComponentStatus(HGM_COMPONENT_WIFI, status);
        vTaskDelay(200);
        this->ComponentStatus(HGM_COMPONENT_NET_TIME, status);
        vTaskDelay(800);
    ...
    lv_anim_set_var(&checkLabelAnim, pb);
    lv_anim_set_values(&checkLabelAnim, 20, -30);
